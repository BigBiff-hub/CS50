...
def buy():
    ...
    if request.method == "POST":
        ...
        if cash < total_price:
            return apology("Insufficent funds transaction failed!")
        else:
...
def register():
    """Register user"""
    if request.method =="POST":
        ## Collect user inputs from HTML Page ##
        ...
        #check if user name has been taken, if is doesn't = 0 means that the username has been used before
        if len(username_query) != 0:
            return apology(f"The username '{username}' has been taken, change slightly please.")
        ## add new user    use generate_password_hash for security
        new_user = db.execute("INSERT INTO users (username, hash) VALUES (?, ?);",
                        username, generate_password_hash(password))
        session["user_id"] = new_user
        flash("Congrats you are Registered for CS50 Finance!")
...
def sell():
    ...
    if request.method == "POST":
        ...
        if current_shares < shares:
            return apology("Not enough shares avaliable")
            
        cash_on_hand = db.execute("SELECT cash FROM users WHERE id = ?", user_id)[0]["cash"]
        db.execute("UPDATE users SET cash = ? WHERE id = ?", cash_on_hand + total_price, user_id)
        db.execute("INSERT INTO transactions (user_id, name, shares, price, type, symbol) VALUES(?, ?, ?, ?, ?, ?)",
        user_id, stock_name, -shares, total_price, "sell", symbol)
        balance = total_price + cash_on_hand
        return render_template("success.html", action="sold", shares=shares,
                                symbol=symbol, balance=usd(balance))
    else:
        user_id = session["user_id"]
